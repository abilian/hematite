...
@pytest.mark.parametrize(
    "input,expected", [("HTTP1/.a", P.InvalidVersion), ("SDF1/1", P.InvalidVersion)]
)
...
@pytest.mark.parametrize(
    ...
    [
        (
            P.StatusLine(
                version=P.HTTPVersion(major=1, minor=1), status_code=200, reason="OK"
            ),
            "HTTP/1.1 200 OK",
        ),
        (
        ...
        (
            P.StatusLine(
                version=P.HTTPVersion(major=1, minor=1), status_code=500, reason=""
            ),
            "HTTP/1.1 500",
        ),
        (
            P.StatusLine(
                version=P.HTTPVersion(major=1, minor=1), status_code=500, reason=None
            ),
...
@pytest.mark.parametrize(
    "input,expected",
    [
        ("GET / HTTP/1.1", P.RequestLine("GET", URL("/"), P.HTTPVersion(1, 1))),
        (
...
@pytest.mark.parametrize(
    ...
    [
        (
            P.RequestLine(
                method="GET", url=URL("/"), version=P.HTTPVersion(major=1, minor=1)
            ),
            "GET / HTTP/1.1",
        ),
        (
...
def test_RequestLine_round_trip():
    ...
    expected = P.RequestLine(
        method="OPTIONS", url=URL("*"), version=P.HTTPVersion(1, 1)
    )
