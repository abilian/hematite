...
from hematite.raw.parser import (
    StatusLine,
    HTTPVersion,
    HeadersWriter,
    ResponseReader,
...
class RawResponse(object):
    def __init__(
        ...
        **kwargs
        ...
        self.content_length = kwargs.pop("content_length", traits.content_length)
        self.connection_close = kwargs.pop("connection_close", traits.connection_close)
        if kwargs:
    ...
    def status_line(self):
        return StatusLine(
            version=self.http_version, status_code=self.status_code, reason=self.reason
        )
    ...
    def get_writer(self):
        return ResponseWriter(
            status_line=self.status_line, headers=HeadersWriter(self.headers), body=[]
        )  # TODO: bodies

    def to_bytes(self):
        writer = self.get_writer()
        return b"".join(
            part for _state, part in writer.writer if _state != Complete.type
        )
